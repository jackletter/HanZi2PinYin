...
namespace HanZi2PinYin
{
    #region 汉字转化成拼音源码

    public class Chinese2Spell
    {
        public string GetSpell(string str_Chinese, ConvertModel model)
        {
            char[] chars = str_Chinese.ToArray<char>();
            string res = "";
            string tmp = "";
            foreach (var item in chars)
            {
                if (System.Text.RegularExpressions.Regex.IsMatch(item.ToString(), "[A-Z]|[a-z]"))
                {
                    res += item;
                }
                else
                {
                    tmp = GetSpell(item.ToString());
                    switch (model)
                    {
                        case ConvertModel.AllCaps:
                            tmp = tmp.ToUpper();
                            break;
                        case ConvertModel.AllLower:
                            tmp = tmp.ToLower();
                            break;
                        case ConvertModel.FirstCap:
...
/*
            ...
            str = c.GetSpell("abc胡12庆d杰e", ConvertModel.AllCaps);
            Console.WriteLine(str);
            str = c.GetSpell("胡庆杰", ConvertModel.AllLower);
            Console.WriteLine(str);
            str = c.GetSpell("15胡u庆H杰_", ConvertModel.FirstCap);
            Console.WriteLine(str);
            #endregion
            Console.ReadLine();
